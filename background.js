...
async function relayAudioCaptionResultToTab(result) {
  ...
  const relayMsg = {
    ...
    source: result?.source === 'audio_stt_disabled'
      ? 'audio_stt_disabled'
      : (result?.source === 'audio_stt_unavailable'
        ? 'audio_stt_unavailable'
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.action === 'updateIcon') {
    ...
    // not let it reset the backend rolling transcript while caption accuracy is
    // being repaired.
    ...
    if (Number.isFinite(tabId)) {
      ...
      // VAD is sent to the page for display/overlay behavior only.
      // It is not used to clear the backend rolling transcript buffer.
...
async function handleAudioCaptionChunk(videoId, audioChunk, mimeType, startSeconds, endSeconds, audioSamples = null, sampleRate = null, channels = null, chunkMeta = null, tabId = null) {
  ...
  try {
    ...
    const result = {
      ...
      no_change: payload.no_change === true,
